...
document.addEventListener("DOMContentLoaded", () => {
  ...
  document.getElementById(
    "active-scroll-id"
  ).style.transform = `translateX(-${leftValue}vw)`;
...
const navigateCarousel = (step, stepType) => {
  ...
  )[0].style.left = `${finalLeftCarousel}%`;
  document.getElementById("active-scroll-id").style.transition =
    "all .5s cubic-bezier(0.25, 0.46, 0.45, 0.94)";
  document.getElementById(
    "active-scroll-id"
  ).style.transform = `translateX(-${finalLeftScroll}vw)`;
